...
foreach (glob($siteConfPath) as $siteConfig) {
    ...
    exec("ln -s /home/vagrant/development/vagrantConfig/siteConfigs/{$pathInfo['basename']} /etc/nginx/sites-enabled/{$pathInfo['basename']}");
}
...
if (array_key_exists('databases', $jsonConfig)) {
    foreach ($jsonConfig['databases'] as $databaseConfig) {
        ...
        if (file_exists("/home/vagrant/development/localStorage/dbBackups/{$databaseConfig['name']}_latest.sql")) {
            exec("mysql -uroot -psecret {$databaseConfig['name']} < /home/vagrant/development/localStorage/dbBackups/{$databaseConfig['name']}_latest.sql");
        }
    }
}
